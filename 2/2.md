...
| Rendimiento         | Ingreso simultáneo de datos por múltiples usuarios     | Empleados                 | Módulo de almacenamiento| Uso en horas de alta demanda              | El sistema debe procesar entradas concurrentes sin generar errores ni bloqueos        | Tiempo de respuesta menor a 3 segundos por operación      |
| Rendimiento         | Consultas simultaneas al sistema                  | Empleados       | Módulo de almacenamiento                  | Horario laboral          | El sistema debe mostrar los resultados rápidamente.                                    | La búsqueda devuelve resultados en menos de 3 segundos    |
| Mantenibilidad      | Verificación de datos     | Empleado de configuración      | Módulo de configuración                   | Durante su ejecución| El sistema permite validaciones sin afectar su obtención de datos                    | Tiempo de validación menor a 5 segundos              |
| Usabilidad          | Registro de datos       | Empleados          | Módulo de gestión de inventario/Comprobación de pago/configuración                   | Uso en horario laboral                 | El sistema debe guiar al usuario con indicadores de correcto ingreso de datos en tiempo real                  | El registro debe ser realizado en menos de 3 minutos |
...
| Reusabilidad        | Reutilización de componentes para agregar categorías, productos y proveedores | Equipo de desarrollo      | Formularios de entrada                  | Proceso de desarrollo      | Los formularios deben seguir un diseño modular reutilizable                           | 80% del código compartido entre formularios               |
| Seguridad           | Ingreso de usuario no autorizado al módulo de empleados | Usuario externo            | Módulo de gestión de empleados          | Intento de acceso no autorizado | El sistema debe denegar el acceso y registrar el intento                              | 100% de intentos no autorizados bloqueados y registrados  |
| Testabilidad        | Cambios en la lógica de generación de comprobantes     | Equipo de QA              | Módulo de comprobantes de pago          | Fase de pruebas            | El sistema debe permitir pruebas automatizadas sobre los nuevos cambios               | Cobertura de pruebas ≥ 90% en el módulo                   |
| Portabilidad        | Migración del sistema a otro equipo con diferente SO   | Administrador de sistema  | Aplicación completa                     | Instalación en nuevo entorno| El sistema debe ejecutarse sin necesidad de cambios mayores                           | Instalación exitosa en menos de 30 minutos                |
| Capacidad de recuperación | Corrupción accidental de archivos de configuración     | Usuario avanzado           | Configuración del sistema               | Error de usuario           | El sistema debe restaurar los valores por defecto o desde un respaldo                 | Configuración restaurada correctamente en menos de 5 min  |
| Mantenibilidad      | Refactorización de módulo de proveedores               | Equipo de desarrollo      | Módulo de proveedores                   | Fase de desarrollo          | El sistema debe permitir realizar refactorizaciones sin afectar módulos relacionados  | No se generan errores colaterales tras el cambio          |
| Eficiencia          | Generación continua de comprobantes en horario pico    | Usuarios concurrentes     | Motor de procesamiento de comprobantes | Alta carga de trabajo      | El sistema debe mantener un desempeño estable sin ralentizaciones                     | No supera el 60% de uso de CPU o memoria                  |
