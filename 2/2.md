...
|---------------------|--------------------------------------------------------|---------------------------|-----------------------------------------|----------------------------|----------------------------------------------------------------------------------------|-----------------------------------------------------------|
| Disponibilidad      | Fallo del sistema de base de datos local               | Computadora central                 | Motor de base de datos                  | Horario laboral           | El sistema muestra mensaje de error y cierra las conexiones. Luego usa el backup                       | El sistema se recupera en menos de 5 minutos              |
| Disponibilidad      | Reinicio inesperado del sistema                        | Sistema operativo         | Aplicación principal                    | Horario laboral            | El sistema guarda el estado actual y reinicia desde el último guardado automático     | No se pierde más de 30 segundos de trabajo                |
| Rendimiento         | Ingreso simultáneo de datos por múltiples usuarios     | Empleados                 | Módulo de creación de clientes/productos| Uso intensivo              | El sistema debe procesar entradas concurrentes sin generar errores ni bloqueos        | Tiempo de respuesta menor a 3 segundos por operación      |
| Rendimiento         | Consulta de productos por categoría                    | Usuario del sistema       | Módulo de productos                     | Operación normal           | El sistema debe mostrar los resultados rápidamente                                    | La búsqueda devuelve resultados en menos de 2 segundos    |
| Seguridad           | Intento de modificación de datos sin permisos          | Usuario sin privilegios   | Base de datos                           | Operación normal           | El sistema debe denegar el acceso y registrar el intento                              | No se permite modificar y se registra el intento          |
| Mantenibilidad      | Actualización del módulo de proveedores                | Equipo de desarrollo      | Módulo de proveedores                   | Fuera del horario laboral  | El sistema permite actualizaciones sin afectar otros módulos                          | Tiempo de mantenimiento menor a 20 minutos                |
| Usabilidad          | Registro de nuevos productos con muchos campos         | Usuario final             | Interfaz de productos                   | Uso diario                 | El sistema debe guiar al usuario con validaciones en tiempo real                      | El registro se completa sin errores en menos de 2 minutos |
| Integridad de datos | Registro de comprobante sin datos de cliente           | Usuario del sistema       | Módulo de comprobantes                  | Operación normal           | El sistema impide crear comprobantes sin información obligatoria                      | No se genera el comprobante y se muestra mensaje claro    |
| Disponibilidad      | Corte de energía inesperado                            | Ambiente físico           | Aplicación y base de datos local        | Horario laboral            | El sistema debe reiniciar sin pérdida de datos tras reestablecerse la energía         | Datos recuperados completamente                          |
| Escalabilidad       | Aumento progresivo de productos registrados            | Crecimiento del negocio   | Base de datos                           | Operación prolongada       | El sistema debe seguir funcionando sin afectar el rendimiento                         | Hasta 100,000 productos sin impacto en el rendimiento     |
